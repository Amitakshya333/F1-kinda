...
class Game {
        ...
        if (this.gameState.isRacing()) {
            ...
            this.cars.forEach((car, index) => {
                ...
                if (car.currentLap > CONSTANTS.TOTAL_LAPS) {
                    // Bug 8 fix: pass the actual car that finished, not always cars[0]
                    this.finishRace(car);
                }
            });
    ...
    togglePause() {
        // Bug 7 fix: check current state first, then transition
        if (this.gameState.isPaused()) {
            this.gameState.resume();
            this.uiManager.hidePauseMenu();
        } else if (this.gameState.isRacing()) {
    ...
    finishRace(winner) {
        ...
        this.gameState.finishRace(winner || this.cars[0]);
